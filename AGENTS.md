...
Tasks are stored in a SQLite database at `~/.tinyclaw/tasks.db` and served via REST API at `http://localhost:8080/tasks`. They appear in the TinyClaw Dashboard at `http://localhost:8080`.

When the user sends a task command, create it via the API:

```
POST http://localhost:8080/tasks
Content-Type: application/json

{ "title": "...", "description": "...", "priority": "high|medium|low", "agent": "tinyboy|unassigned", "status": "inbox", "source": "telegram" }
```

### Recognized task commands (any channel)

| Format | Example |
|--------|---------|
| `/task <title>` | `/task Fix the login bug` |
| `/task <title> @<agent>` | `/task Review PR @coder` |
| `/task <title> priority:<level>` | `/task Deploy hotfix priority:high` |
| `/task <title> @<agent> priority:<level>` | `/task Write docs @copywriter priority:low` |

When you receive a `/task` command:
1. Parse title, agent (`@name`), priority (`priority:high/medium/low`) from the message
2. POST to `http://localhost:8080/tasks` with `source: "telegram"` (or `"discord"` / `"whatsapp"`)
3. Confirm to the user: "✅ Task created: **<title>** → assigned to <agent>, priority <level>"

You can also list, update and delete tasks:
- `/tasks` — list all open tasks (inbox + in progress)
